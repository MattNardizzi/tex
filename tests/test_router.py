...
def test_final_score_respects_fusion_weights() -> None:
    # Set all non-criticality layers to 0 and semantic to 1.0. With the
    # rebalanced default fusion weights (semantic=0.42, criticality=0.08,
    # deterministic=0.28, specialists=0.22), final score should equal
    # 0.42 + (criticality * 0.08) where criticality is bounded to [0, 1].
    # So the achievable range is [0.42, 0.50].
    result = _route(
    ...
    # Deterministic=0, specialists=0, semantic=1.0*0.42=0.42,
    # criticality <= 1.0 * 0.08 = at most 0.08. So final_score must be in
    # [0.42, 0.50]. Allow a small epsilon for float rounding at emit.
    assert 0.41 < result.final_score < 0.51
