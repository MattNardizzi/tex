...
def test_standing_watch_dormancy_and_held_surfacing_wired(monkeypatch):
    ...
    # first-run demo seed the session conftest suppresses by default. The demo
    # tenant is no longer auto-enrolled (real-only by default), so also opt into
    # the dev-only demo standing watch explicitly.
    monkeypatch.setenv("TEX_DISCOVERY_DEMO_SEED", "1")
