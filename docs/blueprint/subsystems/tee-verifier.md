...
- **CrossGuard per-decision nonce binding** (pattern surveyed in arxiv 2604.23280, an AI-identity survey — not a CrossGuard paper) — `decision_bound_nonce` folds `decision_id` into the attestation nonce to defeat cross-decision JWT replay; `LongHaulNonce` extends it with transcript + fleet nonces.
