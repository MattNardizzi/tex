...
The sanction ladder runs Warning, Fine, Credit (restorative),
Suspension. Deterrence is the mechanism-level objective, calibrated by
the condition pS >= Δπ — sanction expected loss must dominate per-round
collusive rent. Equivalently, a Pigouvian correction on actor utility:
u_i^I(a) = u_i(a) - S_i(a).

Restorative paths come in three kinds:
...
# ABDICO rule IDs are stable join identifiers.
...
class Sanction:
    ...
    description
        Human-legible rationale (rendered into Institutional notices).
    cost_to_actor
    ...
    enforcement_action
        The mechanical action the Controller takes. Maps onto the
        sanction ladder:
    ...
    tier
        Sanction tier (1, 2, 3) for fine-type sanctions.
        None for non-fine actions.
    fine_rate
        Fraction of round profits to deduct (e.g. 0.35, 0.75, 1.00 by
        tier). None for non-fine actions.
    fine_floor
        Minimum monetary penalty regardless of round profits.
        None for non-fine actions.
    duration_rounds
        For suspension: how many rounds the actor is removed.
...
class RestorativePath:
    ...
    Restoration is itself part of the graph: sanctions are reversible
    when evidence improves. Three kinds are supported:
    ...
    restoration_kind
        Which of the three supported kinds this path implements.
    condition
        ...
        Fixture defaults: clean_rounds_required=2, credits_required=1,
        decay_window=10.

    TODO(P2): full credit accounting (earned/spent/decay window)
        lives in a future CreditLedger; Thread 12 only models
