...
class AskResponse(BaseModel):
    ...
    # The presence verdict's TIER token ("sealed"/"derived"/"abstain"), or None
    # when presence is not engaged. It is the MONOTONE fold of the answer's
    # per-claim verdicts (AnswerEnvelope.overall_tier) — not the draft, not a
    # "vibe". The client echoes it verbatim to GET /v1/speak?prosody=<token> so
    # the spoken line's perceived confidence equals the verdict. See the honest
    # trust note on the /v1/speak handler.
...
def build_voice_router() -> APIRouter:
    ...
    def ask(
        ...
        return AskResponse(
            ...
            # Read-only hand-off of the presence tier (pure function of the gate's
            # monotone verdict). None when presence isn't engaged. main.py /
            # voice_ask.py are untouched — this only reads outcome.presence, which
            # the gate seam already populates.
