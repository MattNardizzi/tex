...
- **Emergent-norm / collusion** — Bonjour et al. 2022 information-theoretic action-MI collusion signal (the action-side-channel variant, since Tex has no white-box activations); shared-target Z-score convergence. Union-find with path compression.
...
- **Three-dimension drift taxonomy** (`signal_registry.py:575-608`) — semantic/coordination/behavioral axes via max-pooling over a signal→dimension map. Implemented but, like the rest of `evaluate_drift`, unreached in production.

> The docstring citation labels (AAF, Drift-to-Action, the drift-taxonomy attribution, collusion, etc.) and the benchmark detection-delay figure quoted alongside them were **unverifiable and have been removed from the code** — there is no benchmark harness in scope and no test in this directory was read. The *algorithms named* are correctly implemented; the *empirical performance claims* are not substantiated by code here.
