...
class ASITriggerDTO(BaseModel):
    """One signal that contributed to an ASI finding."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    source: ASITriggerSource
    signal_name: str
    score: float = Field(ge=0.0, le=1.0)
    evidence_excerpt: str | None = None

    @classmethod
    def from_domain(cls, trigger: ASITrigger) -> "ASITriggerDTO":
        return cls(
...
class ASIFindingDTO(BaseModel):
    """
    Structured OWASP ASI 2026 finding for API consumers.

    Every ASI category Tex attributes to this decision appears as one
    ASIFindingDTO. The DTO is the public contract for the "Tex is the
    only content layer that attributes every verdict to specific OWASP
    ASI 2026 categories with linked evidence and verdict-influence
    weighting" claim.
    """
    ...
    triggered_by: tuple[ASITriggerDTO, ...] = Field(default_factory=tuple)
    counterfactual: str | None = None

    @classmethod
    def from_domain(cls, finding: ASIFinding) -> "ASIFindingDTO":
        return cls(
            category=finding.category,
            short_code=finding.short_code,
            title=finding.title,
            description=finding.description,
            severity=finding.severity,
            confidence=finding.confidence,
            verdict_influence=finding.verdict_influence,
            triggered_by=tuple(
                ASITriggerDTO.from_domain(trigger)
...
class LatencyBreakdownDTO(BaseModel):
    """Per-stage wall-clock latency for an evaluation, in milliseconds."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    deterministic_ms: float = Field(ge=0.0)
    retrieval_ms: float = Field(ge=0.0)
    specialists_ms: float = Field(ge=0.0)
    semantic_ms: float = Field(ge=0.0)
    router_ms: float = Field(ge=0.0)
    total_ms: float = Field(ge=0.0)
    dominant_stage: str

    @classmethod
    def from_domain(cls, latency: LatencyBreakdown) -> "LatencyBreakdownDTO":
        return cls(
            deterministic_ms=latency.deterministic_ms,
            retrieval_ms=latency.retrieval_ms,
            specialists_ms=latency.specialists_ms,
            semantic_ms=latency.semantic_ms,
            router_ms=latency.router_ms,
...
class EvaluateResponseDTO(BaseModel):
    ...
    asi_findings: tuple[ASIFindingDTO, ...] = Field(
        default_factory=tuple,
        description=(
            "Structured OWASP ASI 2026 findings attributed to this "
            "verdict, with evidence trail and verdict-influence "
            "classification."
        ),
    )
    determinism_fingerprint: str | None = None
    latency: LatencyBreakdownDTO | None = None
    ...
    def from_domain(
        ...
        return cls(
            ...
            asi_findings=tuple(
                ASIFindingDTO.from_domain(item) for item in response.asi_findings
            ),
            determinism_fingerprint=response.determinism_fingerprint,
            latency=(
                LatencyBreakdownDTO.from_domain(response.latency)
                if response.latency is not None
                else None
            ),
            replay_url=response.replay_url,
            evidence_bundle_url=response.evidence_bundle_url,
    ...
    def from_command_result(
        ...
        *,
        ...
        When ``base_url`` is supplied, absolute ``replay_url`` and
        ``evidence_bundle_url`` fields are populated so API consumers can
        click straight through to the audit surfaces.
        """
        ...
        replay_url: str | None = response.replay_url
        evidence_bundle_url: str | None = response.evidence_bundle_url
        if base_url is not None:
            normalized_base = base_url.rstrip("/")
            replay_url = f"{normalized_base}/decisions/{response.decision_id}/replay"
            evidence_bundle_url = (
                f"{normalized_base}/decisions/{response.decision_id}/evidence-bundle"
            )
        ...
        return cls(
            ...
            evidence_bundle_url=evidence_bundle_url,
            policy_version=response.policy_version,
            evidence_hash=response.evidence_hash,
            evaluated_at=response.evaluated_at,
