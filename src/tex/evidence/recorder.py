...
from tex.evidence.seal import EvidenceChainSigner, PQ_SIGNATURE_FIELD
...
class EvidenceRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        chain_signer: "EvidenceChainSigner | None" = None,
    ) -> None:
        ...
        self._mirror = mirror
        # Optional post-quantum chain signer (Layer 5 seal). When None — the
        # default for any directly-constructed recorder — records are written
        # exactly as before (hash-chained, unsigned), so existing behaviour and
        # tests are unaffected. When present, every appended payload carries an
        # embedded, self-verifying `pq_signature` block (see tex.evidence.seal).
        self._chain_signer = chain_signer
    ...
    def record_human_resolution(
        ...
        *,
        ...
        """
        Seal a held decision's resolution by a NAMED human act.

        This is the wire behind "not approved by a spoken 'yes' — sealed by a
        named human act the evidence layer can prove." A held (ABSTAIN)
        decision is resolved by an operator choosing approve / hold / refuse;
        this records that act as its own hash-chained, post-quantum-signed
        evidence row, linked to the decision it resolves. The returned record's
        ``record_hash`` is the anchor the operator walks away with, and its
        payload carries the embedded ``pq_signature`` (when a chain signer is
        configured) so the seal verifies standalone.

        ``verdict`` is the human verdict on the hold — one of
        ``approved`` / ``held`` / ``refused`` — distinct from the machine
        Verdict on the original decision (which is carried for linkage).
        """
        normalized_verdict = (verdict or "").strip().lower()
        if normalized_verdict not in ("approved", "held", "refused"):
            raise ValueError(
                "human resolution verdict must be one of "
                "'approved' / 'held' / 'refused'"
            )
        actor = (resolved_by or "").strip()
        if not actor:
            raise ValueError("record_human_resolution requires a non-empty resolved_by")
    ...
    def _append(
        ...
        *,
        ...
        with self._lock:
            ...
            # Layer-5 seal: embed a self-verifying post-quantum signature into
            # the payload BEFORE hashing, so the hash chain commits to the
            # signature and the signature is taken over the payload sans its own
            # block (non-circular). No-op when no chain signer is configured.
            safe_payload = self._sign_payload(safe_payload)
            payload_json = self._stable_json(safe_payload)
    ...
    def _sign_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        """
        Embed a post-quantum ``pq_signature`` block into ``payload`` when a
        chain signer is configured; return the payload unchanged otherwise.

        Signing failure is best-effort, exactly like the durable mirror: a
        signer error must NEVER block or corrupt the evidence chain. On failure
        the record is written without a signature (honestly unsigned) and the
        error is logged — the chain stays alive rather than refusing to record.
        """
        if self._chain_signer is None:
            return payload
        try:
            block = self._chain_signer.sign_payload(payload)
            signed = dict(payload)
            signed[PQ_SIGNATURE_FIELD] = block
            return signed
        except Exception as exc:  # noqa: BLE001
            _logger.warning(
                "EvidenceRecorder: chain signing failed (%s); "
                "writing record unsigned",
                exc,
            )
            return payload
