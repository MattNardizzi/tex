...
- `estimate_adversary_payoff(drift_signals)` (`:190`) maps drift → g_max, preferring `abc_drift_d_star` (ABC D*) then `bocpd_run_length_posterior`, falling back to raw `drift_delta`, taking the **max** of recognized signals as a deterrent over-estimate (`:255-257`); returns `fallback_g_max` (0.5) when no signal present.
...
- **AIR incident-response lifecycle** — detect/contain/recover/eradicate; the `air_phase_for` mapping tags each intervention so AIR-compatible tooling can join (`engine.py:65-96`). Eradication-rule synthesis (LLM + deterministic) is the "eradicate" phase.
...
- **Restorative justice / sanction ladder** and **embodied-agent recovery framing** — the design references for the restorative executor (`restorative.py:1-35`); their paper-style docstring citations were unverifiable and have been removed.
