...
Historical log of when each claim was added to Tex. Organized by thread.

**This is not the current claims list — read `CLAIMS_CURRENT.md` for that.**

This file exists so:
- You can trace when a capability was wired (useful for blog posts, release notes).
- A new collaborator can read the build arc end-to-end.
- The thread-N → claim mapping is preserved for the build-log narrative.

Every claim shown as "wired" here remains wired in the current system unless
explicitly listed in a known regression note. The authoritative current-state
file is `CLAIMS_CURRENT.md`.
