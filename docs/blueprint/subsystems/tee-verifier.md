...
- **CrossGuard per-decision nonce binding** — `decision_bound_nonce` folds `decision_id` into the attestation nonce to defeat cross-decision JWT replay; `LongHaulNonce` extends it with transcript + fleet nonces.
- **Proof-of-Guardrail / verdict binding** — folds the verdict + policy + input + ledger-prev into the TDX `report_data` (the hardware-signed field), with constant-time `hmac.compare_digest` comparison; the regression target is the "hollow `eat_nonce`" forgery.
...
6. **Heavy reliance on in-flight standards; paper-style citation labels removed.** Docstrings cite 2026 IETF drafts (draft-messous-eat-ai-01, draft-ietf-rats-ear-03, draft-ietf-rats-corim-10) with specific dates, and formerly carried arxiv-preprint labels that were unverifiable and have been removed from the code. The *code structures* implementing them are real and self-consistent. `verdict_binding.py:28-30` itself marks its novelty citation `UNVERIFIED-FROM-MEMORY`.
