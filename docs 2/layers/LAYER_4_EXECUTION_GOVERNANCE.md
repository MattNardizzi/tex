...
| Package | Files | Status | Role |
|---|---|---|---|
| `governance/` | 20 | Mixed — `private_data_exec/ifc` WIRED via `ifc_specialist`; `path_policy/` WIRED via `engine/path_policy_bridge`; `kernel_mcp/`, `stpa_specs/` TEST_ONLY | Deeper governance work; path_policy + ifc now on the live path |
...
- **Path policies (LTLf over the execution path) — WIRED.** `engine/path_policy_bridge.py` brings `governance/path_policy/` into the PDP as a sibling of contracts. Opt-in via `request.metadata["path_policy"]` (policies + prior trace + candidate action); inert/zero-cost when absent. `block` severity → hard FORBID floor (short-circuits the router); `warn` → soft, promotes PERMIT→ABSTAIN; `audit` → finding only. Closes the per-action blind spot: the PDP now judges an action by the *sequence* it occurs in, not in isolation.
- **Conformal Risk Control verdict gate — WIRED.** `engine/crc_gate.py` puts a finite-sample, distribution-free bound on the false-permit rate (RCPS / Hoeffding-Bentkus; Bates et al. JACM 2021). Sits on the final verdict as the last touch. Monotone-safe: only ever demotes PERMIT→ABSTAIN when the fused score lies outside the certified permit region; never relaxes FORBID/ABSTAIN, never creates a PERMIT. Inert (pass-through) until an operator supplies a calibration set, so pre-CRC behaviour is preserved bit-for-bit. Every decision carries an auditable `CRCCertificate` (alpha, delta, cutoff, empirical risk, UCB, certified false-permit rate, calibration size, bound method) under `decision.metadata["pdp"]["crc"]`.
- **Structural FORBID floor — WIRED.** `specialists/structural_floor.py` promotes the four *deterministic structural* specialists — PCAS (Datalog deny), CaMeL (capability denial), IFC (typed flow violation), ARGUS (`OBSERVATION_DRIVEN` counterfactual proof) — off the diluted voting tier. When one emits its unambiguous deterministic-deny signature (never a merely high score), the PDP short-circuits to FORBID alongside the deterministic gate. Fixes the tier inversion where a proven PCAS deny on otherwise-clean content used to land at ABSTAIN (fused ~0.2 against `forbid_threshold` 0.72). Recorded at `decision.metadata["pdp"]["structural_floor"]`.
- **Adaptive red-team harness — BUILT.** `adversarial/adaptive.py` + `__main__.py` replace static-fixture evaluation with an adaptive "attacker-moves-second" search (Nasr et al. arXiv:2510.09023): a black-box, query-budgeted hill-climb over intent-preserving content mutations that drives a blocked seed toward PERMIT. Reports static vs adaptive ASR, per defense class, and runs as a CI gate (`python -m tex.adversarial`). **First-run finding:** the lexical/deterministic layer is ~100% adaptively bypassable (a single leetspeak mutation evades the regex + lexical specialists), while the structural class holds at 0% — empirically confirming why the structural floor matters and that input canonicalization before the lexical layer is the next hardening target.
...
- **Governance subpackages 2 of 4**: `kernel_mcp/syscall_gate.py` (771 lines!), `stpa_specs/` are tested but never invoked by the runtime. (`path_policy/` is now WIRED via `engine/path_policy_bridge.py`.)
...
### 3. Wire path policies ✅ DONE
`governance/path_policy/` implements LTLf-checked path policies. Customer use case: "this agent may only invoke search_db AFTER receiving user input AND BEFORE invoking send_email." **Wired** via `engine/path_policy_bridge.py` as a sibling check to contracts (see Current state). Opt-in through `request.metadata["path_policy"]`; block→FORBID, warn→ABSTAIN, audit→finding.
