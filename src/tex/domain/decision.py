...
class Decision(BaseModel):
    ...
    asi_findings: list[ASIFinding] = Field(
        ...
        description=(
            "Structured OWASP ASI 2026 findings attributed to this decision."
        ),
    )
    determinism_fingerprint: str | None = Field(
        ...
        max_length=64,
        description=(
            "Stable SHA-256 fingerprint over (content_sha256, policy_version, "
            "deterministic firings, specialist scores, semantic dimension "
            "scores). Two evaluations with the same fingerprint should "
            "produce the same verdict."
        ),
    )
    latency: LatencyBreakdown | None = Field(
        default=None,
        description="Per-stage wall-clock latency for the pipeline that produced this decision.",
    )
